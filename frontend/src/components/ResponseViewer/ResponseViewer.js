  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-medium mb-4">响应</h3>
        <div className="flex justify-center items-center h-32">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
          <span className="ml-3">请求中...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="p-6">
        {activeTab === 'body' && (
          <div>
            <div className="flex justify-between items-center mb-2">
              <h4 className="text-md font-medium">响应体</h4>
        ...
        {activeTab === 'headers' && (
          <div>
            <h4 className="text-md font-medium mb-2">响应头</h4>
            <div className="border border-gray-200 rounded-md">
              <div className="p-4 space-y-2">
                {response.headers && Object.entries(response.headers).map(([key, value], index) => (
                  <div key={index} className="flex">
                    <div className="w-1/3 text-gray-600 font-mono">{key}:</div>
                    <div className="w-2/3 font-mono">{value}</div>
                  </div>
                ))}
                {!response.headers && <p className="text-gray-500">无响应头信息</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'info' && (
          <div>
            <h4 className="text-md font-medium mb-4">请求信息</h4>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-sm text-gray-500">状态码</div>
                <div className="font-medium">{response.status}</div>
              </div>
              <div>
                <div className="text-sm text-gray-500">状态文本</div>
              ...
              <div>
                ...
                <div className="font-medium">{response.size} bytes</div>
              </div>
            </div>
          </div>
        )}
